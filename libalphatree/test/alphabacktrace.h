...
struct TransactionSign{
    ...
    //int64_t dayIndex;
    //这只股票上次交易信息索引
    int64_t preTransactionIndex;
    //下次交易相信索引
    int64_t nextTransactionIndex;
};
...
protected:
    ...
    //得到当前还继续持有的多仓或者空仓
    inline int64_t* getHoldStock(bool isBuy){
    ...
    static int getHoldId(int stockIndex, DArray<TransactionSign, MAX_TRANSACTION_NUM>& transactions,const int64_t* holdStock, int holdSize){
        int holdId = -1;
    ...
    //得到订单中还没卖掉的股票笔数
    inline static int getRemainStockNum(DArray<TransactionSign, MAX_TRANSACTION_NUM>& transactions, int64_t lastIndex){
        int stockNum = 0;
        while (lastIndex >= 0){
    ...
    inline pair<float,float> getSellReward(int sellNum, float price, DArray<TransactionSign, MAX_TRANSACTION_NUM>& transactions, int64_t lastIndex, bool isSellBuy){
        ...
        if(isSellBuy){
            ...
            //找到最早的做空单据和平仓单据
            int64_t sellIndex = transactions[lastIndex].isSell ? lastIndex : -1;
            int64_t shortIndex = !transactions[lastIndex].isSell ? lastIndex : -1;
            int64_t preIndex = lastIndex;
    ...
    int64_t holdBuyStock_[MAX_HOLD_STOCK_NUM];
    int holdBuySize_ = {0};
    int64_t holdShortStock_[MAX_HOLD_STOCK_NUM];
