...
//返回range范围内的最小值的id
inline void _tsMinIndex(float *dst, const float *src, size_t historySize, size_t elementSize, int range){
    ...
    for(int i = 1; i < historySize; ++i) {
        for (int j = 0; j < elementSize; j++) {
            ...
            if (lastId >= i - range) {
                ...
                dst[i * elementSize + j] = minId + 0.0001f;
            }
        }
    }
}
inline void _tsMaxIndex(float *dst, const float *src, size_t historySize, size_t elementSize, int range){
    memset(dst,0, sizeof(float) * elementSize);
    for(int i = 1; i < historySize; ++i) {
        for (int j = 0; j < elementSize; j++) {
            int lastId = (int) dst[(i - 1) * elementSize + j];
